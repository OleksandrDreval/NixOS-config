...
## Безпечні налаштування ядра через sysctl

Цей розділ описує ключові параметри ядра, які налаштовуються через `sysctl` для підвищення безпеки системи. Кожен параметр має конкретну мету та допомагає зменшити ризики атак.

### Захист файлової системи

Ці параметри забезпечують захист файлової системи від різних типів атак:

*   **`fs.protected_fifos = 2`**: Захищає FIFO файли від перезапису. Це запобігає атакам через іменовані канали.
*   **`fs.protected_hardlinks = 1`**: Забороняє створення жорстких посилань на файли, які не належать користувачу. Це запобігає маніпуляціям з файлами.
*   **`fs.protected_regular = 2`**: Захищає звичайні файли від перезапису. Це особливо важливо для системних файлів.
*   **`fs.protected_symlinks = 1`**: Захищає символічні посилання від маніпуляцій. Це запобігає атакам через симлінки.

### Захист ядра

Ці параметри забезпечують захист ядра від різних типів атак:

*   **`kernel.core_pattern = |/bin/false`**: Забороняє створення core dump файлів. Це запобігає витоку конфіденційних даних.
*   **`kernel.dmesg_restrict = 1`**: Обмежує доступ до повідомлень ядра (dmesg). Це запобігає витоку системної інформації.
*   **`kernel.kptr_restrict = 2`**: Обмежує доступ до адрес ядра. Це ускладнює використання атак, що базуються на знанні адрес ядра.
*   **`kernel.randomize_va_space = 2`**: Рандомізує адресний простір. Це ускладнює використання атак, що базуються на передбаченні адрес пам'яті.

### Захист мережі

Ці параметри забезпечують захист мережі від різних типів атак:

*   **`net.ipv4.conf.all.accept_redirects = false`**: Забороняє прийом ICMP redirects. Це запобігає атакам маршрутизації.
*   **`net.ipv4.conf.all.accept_source_route = 0`**: Забороняє source routing. Це запобігає спуфінгу IP-адрес.
*   **`net.ipv4.tcp_syncookies = 1`**: Вмикає SYN cookies для захисту від SYN flood атак.
*   **`net.ipv6.conf.all.accept_ra = 0`**: Забороняє прийом Router Advertisements. Це запобігає несанкціонованій конфігурації мережі.

### Захист пам'яті

Ці параметри забезпечують захист пам'яті від різних типів атак:

*   **`vm.mmap_min_addr = 65536`**: Встановлює мінімальну адресу для mmap. Це запобігає атакам через нульові сторінки.
*   **`vm.mmap_rnd_bits = 32`**: Рандомізує адресний простір. Це ускладнює використання ROP-атак.
*   **`vm.unprivileged_userfaultfd = 0`**: Забороняє використання userfaultfd для непривілейованих користувачів. Це запобігає використанню userfaultfd для атак.

### Інші параметри

*   **`kernel.unprivileged_bpf_disabled = 1`**: Забороняє використання BPF для непривілейованих користувачів. Це запобігає використанню BPF для атак.
*   **`kernel.yama.ptrace_scope = 2`**: Обмежує використання ptrace. Це запобігає використанню ptrace для атак.

Ці параметри забезпечують комплексний захист системи від різних типів атак, зменшуючи ризики витоку інформації та несанкціонованого доступу.
