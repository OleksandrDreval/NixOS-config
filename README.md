...
#### systemd-initrd 
- **Переваги**:
  - Підтримка TPM 2.0 для розкриття ключів ( Конфігурація не підтримує TPM через фундаментальні проблеми цієї платформи на портативних пристроях «Ваші мізки набагато надійніші» для робочих станцій і серверів рекомендується Апаратний модуль безпеки (HSM) )
